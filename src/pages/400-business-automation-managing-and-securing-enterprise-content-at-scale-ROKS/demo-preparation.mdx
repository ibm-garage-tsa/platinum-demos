...
**WARNING AS OF February 9, 2023**: Due to an issue with OCP 4.10.x on Tech Zone, Cloud Pak for Business Automation reservations are randomly failing. If you are unable to find the icp4adeploy-cp4ba-access-info ConfigMap in the setup below (Step 1.11), you will know the deployment has failed. If that happens, please delete the failed environment and make a new reservation.
