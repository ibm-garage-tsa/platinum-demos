...
In this demo, I’ll show you how Cloud Pak for Watson AIOps helps SREs and IT Ops quickly identify, diagnose, and resolve incidents across mission-critical workloads.
<br/>
You’ll see how:<br/><br/>
-	Watson AIOps intelligently correlates multiple disparate sources of information such as logs, metrics, events, and topology;<br/>
...
<summary>2 - Getting notification of an emerging problem</summary>
...
| :--- | :--- |
| **Action** | Scroll to the “relevant events” section of story |
...
| **Action** | Click the **relevant events** button at the bottom of the notification<br/><br/>A pop-up will appear with the grouped events<br/>
...
| :--- | :--- |
| **Actions** | Close the log anomaly pop-up<br/><br/>Click on “similiar incidents” button<br/><br/>Type “rating” into the search box |
...
| **5.1** | **View Event Manager to determine the root cause and fix problem** |
| :--- | :--- |
| **Actions** | Navigate to **Event Manager (formally known as NOI)**<br/><br/>Click on **Events** in the sidebar menu<br/><br/>The top event should show a red circle (**critical**)<br/><br/>Click the down arrow to expand it |
...
| **Actions** | Click on the second row to select it (rating service mem usage) summary column<br/>Click on **Execute runbook**<Click on **Start runbook**<br/>Click on **Run** |
| **Narration** | Now we’ll execute the runbook that should resolve the underlying failure the Rating service. That should fix the problem. |
...
| :--- | :--- |
| **Actions** | Go back to Runbook screen<br/><br/>If you navigated away from it, get back with these steps:<br/><br/>Automations -> Runbooks -> click three dots at far right for the “Reset Rating Service” runbook -> Choose “view history” -> Click resume on most recent<br/><br/>Scroll down to bottom where there are stars/rating |
...
| **Actions** | At the bottom of the story notification, click **acknowledge**<br/><br/>Choose **keep in channel** and **close**<br/><br/> Optionally add a comment such as “executed runbook to fix issue,” then click **acknowledge* |
