...
| :--- | :--- |
| **Narration** | The teams see a hierarchical view of the invocation, all the way from the end-users' web browser to the backend system. There are a number of sub-calls traced (1), and several are experiencing an undesirable delay and/or error. The timeline (2) plots the duration of each sub-call, immediately highlighting the error details (3). |
