---
title: Using Instana to monitor integration deployment
  300-level live demo
description: Using Instana to monitor integration deployment 300-level live demo
tabs: [ 'Demo preparation', 'Demo script']
---

export const Title = () => (
  <span>
      <h1 style="font-size:60px;"> Using Instana to monitor integration deployment<br/> 300-level live demo</h1>
  </span>
);

![banner](./images/AIOps_Observability_300_Script.jpg)

<span id="top"></span>

<details>

<summary>Introduction</summary>

<br/>

Today we will see how a bank can visualize their cloud native applications and integration estate. This includes understanding the health of the infrastructure, components, and live transactions. 

<br/>

We’ll see how the bank is alerted to a live production issue. They will visualize the banking application that is experiencing the issue, with auto-correlated events highlighted within the context of that application. The team will drill down into the root cause and understand how to resolve the issue and prevent it from happening in the future. 

<br/>

Let’s get started.

<br/>

**[Go to top](#top)**

</details>

<details>

<summary>1 - Viewing your integration deployments</summary>

<br/>

| **1.1** | **Understand the Instana platform ** |
| :--- | :--- |
| **Narration** | Focus Bank uses microservices architecture for their application development. IBM MQ provides the communication, and IBM App Connect Enterprise the glue between microservices. The SRE (Site Reliability Engineer) and the application and integration teams need to be able to observe the environment and be alerted to issues that occur. They need to work together, collaboratively, to assure the Bank’s applications are available.<br/><br/>IBM Instana provides a full-stack observability platform, which gives deep insights for the SRE, application and integration teams. The home screen shows five categories, which act as launch points based on your role:<br/><br/>1.	**Website & Mobile Apps** shows the end user interaction and their experiences.<br/><br/>2. **Applications** focuses on the server-side of the application, such as the number of calls, latency, and error call rate. <br/><br/>3. **Platform** shows the target deployment environments that are used for the applications, such as Kubernetes, Cloud Foundry and vSphere.<br/><br/>4.	**Infrastructure** shows with the physical and virtualized servers underpinning the applications.<br/><br/>5. **Events** highlights the incidents that the operational team should be reviewing. |
| **Action** &nbsp; 1.1.1 | Show the Instana home screen, pausing to explain each category: **Websites & Mobile Apps** (1) and **Applications** (2). <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-1-1-instana-home-screen.png" width="800" /> |

| **1.2** | **View the entire network ** |
| :--- | :--- |
| **Narration** | Let’s look at Focus Bank’s infrastructure. <br/><br/>Instana provides an infrastructure map across the entire estate. This is automatically discovered by the Instana agents, with additional user categorization possible. <br/><br/>In Instana, servers are referred to as hosts. These hosts include those used for the integration technology as well as the microservices that make up the application. This view is not limited to a single data center - it is a global hybrid multi-cloud view where hosts are on-premises and in public cloud environments.<br/><br/>If we click on one of the hosts, this will show the hardware specification, immediately highlighting any issues with the infrastructure. In this case, there are no issues. Scrolling down we can see the components installed. In this case you can see IBM MQ, IBM App Connect Enterprise and several others. <br/><br/>Let’s click into the host dashboard to take a deeper look. |
| **Action** &nbsp; 1.2.1 | Click on the infrastructure icon.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-2-1-infrastructure-icon.png" width="800" /> |
| **Action** &nbsp; 1.2.2 | Point out that the servers are configured for this Instana instance. Point out the servers could include various categories across on-premises and the cloud.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-2-2-servers.png" width="800" /> |
| **Action** &nbsp; 1.2.3 | Zoom into the **cp4i** category and hover over the servers, hovering your mouse over each server to show the components. <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-2-3-cp4i-category.png" width="800" /> |
| **Action** &nbsp; 1.2.4 | We will show the IBM MQ dashboard. Stop when you see **IBM MQ Queue Manager** within the list.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-2-4-mq-dashboard.png" width="800" /> |
| **Action** &nbsp; 1.2.5 | Click on the server with IBM MQ and scroll down to view the components installed on the server. <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-2-5-components.png" width="800" /> |
| **Action** &nbsp; 1.2.6 | Click on Open Dashboard. <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-2-6-open-dashboard.png" width="800" /> |

| **1.3** | **Examine IBM MQ’s metrics ** |
| :--- | :--- |
| **Narration** | Instana displays details, such as the operating system and hostname. Live and historical charts are available for various metrics such as CPU, memory, network and disk utilization.<br/><br/>The software components running on the host can also be seen. Let’s take a closer look at one of these. |
| **Action** &nbsp; 1.3.1 | Expand the **IBM MQ Queue Manager** (1) section and click on the link (2).<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-3-1-link.png" width="800" /> |
| **Narration** | Here we can see the details for the IBM MQ Queue Manager. This is the default view provided automatically. <br/><br/>The MQ agent understands how to collect the metrics, and what views will be useful for the user. The live and historical metrics show the number of connections into the queue manager, the number of messages, and the configured MQ objects. If we look at the queues table, we can quickly filter and see the current state, such as the number of messages on the queue, and the time of the last put and get. We can dig deeper into all the queues, topics and applications accessing the queue manager, and see the details. |
| **Action** &nbsp; 1.3.2 | <inlinenotification>If the IBM MQ instance starts with **corebanking** (1), search for <strong>BALANCE</strong> queue in the next step. Otherwise, search for <strong>AUDIT</strong>.</inlinenotification> Scroll down to the **Queues** table (2), and type either **balance** or **audit** into the Search bar (3). <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-3-2-search.png" width="800" /> |
| **Action** &nbsp; 1.3.3 | Click on the top queue.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-3-3-click-queue.png" width="800" /> |
| **Action** &nbsp; 1.3.4 | Point out the details for the queue.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/1-3-4-details.png" width="800" /> |
  
<br/>

**[Go to top](#top)**

</details>

<details>

<summary>2 - Observing an application issue</summary>

<br/>

| **2.1** | **Review the application  ** |
| :--- | :--- |
| **Narration** | Focus Bank’s SRE team have just received an alert from Instana that there has been a sudden increase in errors on their Banking application.<br/><br/>The alert shows up in the teams' Slack channel, but it can also be configured to show up in PagerDuty, Microsoft Teams, and many others (full list link needed).<br/><br/>Instana only sends alerts when end users are impacted, so the SREs know this is an issue that needs their attention.<br/><br/>The SRE team loads the Banking App summary tab and immediately sees that the erroneous call rate and latency has increased. Clearly something has suddenly started to go wrong. <br/><br/>The SRE team arranges a call with the application and integration teams so they can investigate together. Everyone on the call has a different level of knowledge of the application, so the team looks at the component topology. <br/><br/>They see the 'bankUI' microservice is where the communication starts, and this calls six separate request flows:<br/>• Transfer API – implemented using App Connect<br/>• Balance API – implemented using App Connect<br/>• Audit API – implemented using App Connect<br/>• Notification – implemented using NodeJS<br/>• Fraud – implemented using Java in OpenLiberty<br/>• Authentication – implemented using Java in OpenLiberty<br/><br/>Some of these depend on further downstream components such as IBM MQ and DB2. All of these components and their relationships are automatically discovered and visually displayed. This is made possible by Instana tracking every request that flows through the application.<br/><br/>They can tell there are problems with the application because several services are highlighted in yellow. |
| **Action** &nbsp; 2.1.1 | Change the time period Instana is showing to 45 minutes past the hour, for a 15 minute window (1). Click **Set Time** (2).<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/2-1-1-set-time.png" width="800" /> |
| **Action** &nbsp; 2.1.2 | Click on the **Application** icon (1), and select **Banking App** (2) from the table.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/2-1-2-banking-app.png" width="800" /> |
| **Action** &nbsp; 2.1.3 | Show the **Erroneous Call Rate** (1) and **latency** (2) graphs. Click on the **Dependencies** (3) tab.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/2-1-3-graphs-dependencies.png" width="800" /> |
| **Action** &nbsp; 2.1.4 | Click through the first two columns of icons, pausing to show the technology being used and the summarized view. <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/2-1-4-summarized-view.png" width="800" /> |
 
<br/>

**[Go to top](#top)**

</details> 

<details>

<summary>3 - Inspecting auto-correlated incidents</summary>

<br/>

| **3.1** | **Examine the issue ** |
| :--- | :--- |
| **Narration** | The SRE team clicks on the issue to inspect in more detail.<br/><br/>They see all the issues corelated together for the current incident on the events page. Instana classifies events into three categories:<br/>1. Changes, for example a configuration change that may have occurred in the environment.<br/>2. Issues, that are identified within an individual component.<br/>3. Incidents, correlate issues and changes automatically together.<br/><br/>The teams can see that the incident is still active, and the triggering event is still highlighted. A graphical view of the error is shown, explaining when the error started and the rate of failure. |
| **Action** &nbsp; 3.1.1 | Click on the **Issues** button (1) and select **Sudden increase in the number of erroneous calls** (2).<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/3-1-1-sudden-increase.png" width="800" /> |
| **Action** &nbsp; 3.1.2 | Show the issue is still open (1), and click the **Analyze Calls** button (2). <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/3-1-2-analyze-calls-button.png" width="800" /> |
 
<br/>

**[Go to top](#top)**

</details>

<details>

<summary>4 - Drilling down into the root cause</summary>

<br/>

| **4.1** | **View the calls via the visual dashboard ** |
| :--- | :--- |
| **Narration** | The team navigates into the effected API calls to drill down into the cause. <br/><br/>Instana traces all of the calls, ensuring nothing gets missed. This allows the team to analyze all the calls during the failure and ensure nothing is lost. <br/><br/>They see a timeline and table for all the calls experiencing failures. The team selects one of the invocations for further analysis. |
| **Action** &nbsp; 4.1.1 | Expand the **GET /api/balance** row, showing all the affected calls.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-1-1-affected-calls.png" width="800" /> |
| **Action** &nbsp; 4.1.2 | Click on one of the calls.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-1-2-click-call.png" width="800" /> |

| **4.2** | **Understand the impact and source of the incident ** |
| :--- | :--- |
| **Narration** | The teams see a hierarchical view of the invocation, all the way from the end-users web browser to the backend system. There are a number of sub-calls traced, and several are experiencing an undesirable delay and/or error. The timeline plots the duration of each sub call, immediately highlighting where the issue may be located. |
| **Action** &nbsp; 4.2.1 | Highlight the aspects mentioned in the narration. <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-2-1-highlight.png" width="800" /> |
| **Narration** | The team expand the call, allowing them to drill down and look at each step of the invocation. Within some components they can see sub-calls within an individual component, for instance individual input / output nodes in App Connect Enterprise. <br/><br/>The calls are automatically color labeled - one for HTTP, MQ and JDBC traffic. As they look closer, they can identify the components that are bridging multiple protocols as they have multiple colored bars. <br/><br/>At the top of the stack, they see that 503 – Service Unavailable is being returned to the calling application, with an approximate 1 second latency. They initially focus on the App Connect Enterprise calls and see the message is put quickly onto the IBM MQ BALANCE.IN queue, to retrieve the balance, and immediately waits for a response message on the BALANCE.OUT queue. This call fails after one second, so initially they wonder if it might be an App Connect issue, but the integration team can immediately look deeper. <br/><br/>They look at the emitted event, and the MQ return code is 2033, which means ‘no message available’. App Connect tried to retrieve a response message for one second, and an error is returned when none were available. The teams agree that this suggests the issue is deeper into the call, so continue to dig into the call stack. <br/><br/>At the bottom of the call stack they can see an error connecting to the database from the OpenLiberty Java application. The application team immediately investigates the issue, and they are able to quickly resolve it. An issue that initially appeared to be a high latency in App Connect turned out to be an underlying Database issue. All of this was discovered automatically by Instana. |
| **Action** &nbsp; 4.2.2 | <inlinenotification>As this is a live system the number of sub-steps and the exact error message reported by database may be different.</inlinenotification><br/><br/>Scroll down to the call stack and expand each step. Highlight the transport coloring and the high level errors.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-2-2-scroll-down.png" width="800" /> |
| **Action** &nbsp; 4.2.3 | Collapse and click on the **Balance.get... MESSAGING** call. As highlighted in the narration, explain how App Connect successfully places the message on IBM MQ. <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-2-3-balance-get.png" width="800" /> |
| **Action** &nbsp; 4.2.4 | Click on the **! Balance.gen** call (1). On the right hand side scroll down (2) and show the **messaging.ibmmq.return_code** value (3). As highlighted in the narration, explain how MQ did not receive a response message within a second so timed out. <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-2-4-click-call.png" width="800" /> |
| **Action** &nbsp; 4.2.5 | Expand the call stack, and click on the **CONNECT** (1) call. As highlighted in the narration explain how the database is unavailable (2), and this error takes three seconds to be returned. <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-2-5-connect-call.png" width="800" /> |
 
| **4.3** | **See the issue is resolved ** |
| :--- | :--- |
| **Narration** | Now that Instana have identified the root cause, the application team resolved the issue and they view the live transactions flowing through the application.<br/><br/>Now the teams see API calls taking tens of milliseconds and completing successfully. |
| **Action** &nbsp; 4.3.1 | Expand the **Time Range** section (1), then change the time period Instana is showing to on the hour, for a 30 minute window (2). Click **Set Time** (3) <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-3-1-change-time.png" width="800" /> |
| **Action** &nbsp; 4.3.2 | Click on the **Application** icon (1)and select the **Banking App** row (2).  <br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-3-2-banking-app.png" width="800" /> |
| **Action** &nbsp; 4.3.3 | Show how the errors and latency ratios have returned back to normal.<br/><br/> <img src="https://raw.githubusercontent.com/ibm-garage-tsa/platinum-demos/master/src/pages/300-observing-your-integration-estate-with-instana/images/4-3-3-normal.png" width="800" /> |
 
<br/>

**[Go to top](#top)**

</details>

<details>

<summary>Summary</summary>

<br/>
You’ve seen how Instana can help make the process of identifying problems and finding the root cause of those problems frictionlessly across technology stacks. Since Instana automates so many of the manual and labor-intensive aspects of the process, you can focus on getting other work done and not worry about instrumenting observability or constantly monitoring for problems. And when problems do arise, all the trace data is there at your fingertips to dig into.

<br/>

Thank you for attending today’s demonstration.

<br/>

**[Go to top](#top)**

</details>