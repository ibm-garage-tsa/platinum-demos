...
<summary>1 - Create a ROKS cluster on IBM Cloud</summary>
...
1. Go to IBM Tech Zone to open the <a href="https://techzone.ibm.com/collection/cloud-pak-for-integration-activation-kit-v-2/" target="_blank" rel="noreferrer">Cloud Pak for Integration Activation Kit v2</a> and log in using your IBM credentials.
...
3. Select the ROKS cluster that you want (the simplest way is to select the latest version of the Express installation).
...
4. Complete the **Create a reservation** form to define the details of your ROKS Reservation. Select the "Schedule for later" option, but do not change the date / time details.
...
6. In approximately thirty minutes, you will recieve an email confirming your ROKS environment is ready. You are now ready to install Cloud Pak for Integration.
...
1. Log in to <a href="https://cloud.ibm.com/catalog" target="_blank" rel="noreferrer">https://cloud.ibm.com/catalog</a> using your personal credentials. Confirm that you are using the **ITZ** account.
...
3. Note that your deployment target and delivery method may be already selected. If not, on **Select a deployment method**, select the version of Cloud Pak for Integration that you want to install. Since some demos are ready for a specific Pak version, please check what version is necessary on the demo page.
...
7. Scroll down to the **Set the deployment values** section and deselect the **Use an existing secret?** option to define a new password. Enter a value for csDefaultAdminPassword. It is important that you choose a password that is 32 characters in length and is composed of only letters and numbers. You will need this password in step 13.
...
8. Scroll down to **Parameters with default values**. Click the collapsible section to open them. Change the default values parameters listed below.
...
13. Log in using the admin userid and password you created in step 7.
