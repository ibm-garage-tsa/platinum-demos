...
Our example scenario focuses on an insurance broker who gathers insurance quotes from multiple companies and provides an aggregated list to customers via an API. This involves multiple integration styles, including application integration, API management, and messaging. We will build up to automated deployment and operations for the whole solution.

<br/>

Let's begin with a simple deployment of a single integration from our solution - an integration that retrieves a quote from an insurer. We’re going to deploy this integration using cloud native techniques on containers. You will see that you don’t need to know anything about containers, Kubernetes or OpenShift (Red Hat’s Kubernetes platform) to  deploy a simple integration.  
...
| **Narration** | Using login credientials, let's log in to a cloud version of the product on IBM Cloud. |
...
| :--- | :--- |
| **Narration** | Welcome to IBM Cloud Pak for Integration! This is the home screen, which shows all the capabilities of the pak in one place. Specialized integration capabilities for API management, application integration, messaging, and more, are built on top of powerful automation services. Let’s see the integration capabilities available. |
