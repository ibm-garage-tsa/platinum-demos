...
Later, we will explore a more complex integration solution, with multiple  integration capabilities that we want to deploy together - including integration flows, queues, and managed APIs.
...
| :--- | :--- |
| **Narration** | We’ll create an integration server for us to deploy our new integration. These are being deployed to containers, and each one of these integration servers will be in a separate container, but you don’t actually need to know that to be able to do this simple deployment. |
