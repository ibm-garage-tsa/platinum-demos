...
1. Click <a href="https://cloud.ibm.com/catalog/content/ibm-cp-integration-72f63273-f2f6-4e9c-8626-60fe798c57be-global" target="_blank" rel="noreferrer">here</a> to install Cloud Pak for Integration. This link takes you to an IBM Cloud page. Log in with your personal IBM credentials. <br/>
...
13. Log in using the Username **“admin”** and the 32-character password that you created previously in step 5.
...
This demo uses a Tekton pipeline to build and deploy images in four App Connect integration servers and one MQ server. The GitHub repository includes a script to create the pipeline, so you must clone it to your local machine.
...
3. You should now see a folder and multiple files. To be able to execute our script files, change the file permission with this command:
...
1. Open the **cp4i-deployment-samples folder**.<br/><br/><code>cd cp4i-deployment-samples</code>, <br/><br/>then open the **DrivewayDentDeletion/Operators folder**.<br/><br/><code>cd DrivewayDentDeletion/Operators</code><br/><img src="./images/prepare-6-1.png" width="800" /><br/>

2. Create the pipeline by running this command.<br/><br/><code>./cicd-apply-test-apic-pipeline.sh -n $NAMESPACE -r $REPO -b $BRANCH</code><br/><img src="./images/prepare-6-2.png" width="800" /><br/>

3. In the terminal output, copy the "curl" command and make a note of it. You will need it during the demo to trigger the pipeline. It should look something like the following, although note this is just a sample, the URL for your cluster will be different:
  
<br/>

```
curl -X POST http://el-main-trigger-route-cp4i.itzroks-xyz.eu-de.containers.appdomain.cloud --header "Content-Type: application/json" --data '{"message":"Test run"}'
```

<br/>

4. You may want to run the curl command now to run the pipeline for the first time. The pipeline always runs more quickly on subsequent runs so you will not have to wait as long for the pipeline to complete in your actual demo. 

<br/>

5. Return to the OpenShift console. On the left menu, open **Pipelines**, then **Pipelines** to see what has been added.
<br/><img src="./images/prepare-6-3.png" width="800" /><br/>

6. On the **Project combo box** (1), select **cp4i** (2).
<br/><img src="./images/prepare-6-4.png" width="800" /><br/>

7. Validate there is a pipeline.
