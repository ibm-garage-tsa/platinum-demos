...
This demo uses AI and low-code integration to synchronize contact information between the Salesforce and Insightly CRM systems. We will recognize and automatically match fields with high confidence levels. We recommend an assist with other fields with lower confidence. The Mapping Assist capability learns from previously accepted matches to raise confidence levels in the future.
...
| **Narration** | Here we have our demo flow.<br/><br/> In the designer flow editor, we can edit and change our flow. Before we start to create our flow, let's adjust the response. This response serves to close the API flow. Because there is no one waiting for the API response, we can have a really simple response. We just need to have a response ID. |
...
<details>
<summary>4 - The Mapping Assist feature </summary>
